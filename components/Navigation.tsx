  ...
  return (
    <>
      ...
      {activeDropdown && dropdownPos && (
        <div className="fixed w-48 bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-gray-100 dark:border-neutral-800 py-2 z-[60]" style={{ top: dropdownPos.top, left: dropdownPos.left }}>
          {(activeDropdown === 'Negocios' ? businessFilters : artFilters).map(f => (
            <button key={f} onClick={() => { setSelectedTopic(`${activeDropdown}-${f}`); setActiveDropdown(null); }} className="w-full text-left px-5 py-2.5 text-sm hover:bg-gray-50 dark:hover:bg-neutral-800 text-gray-700 dark:text-neutral-300 hover:text-brand-orange">
